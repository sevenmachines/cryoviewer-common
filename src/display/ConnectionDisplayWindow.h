...
protected:
	...
	virtual void updateComponentDisplay(
			...
			std::map<boost::uuids::uuid, boost::shared_ptr< ConnectionActivityPanel > > & panelsMap,  const ComponentType & type);
	...
	boost::shared_ptr<ConnectionActivityPanel> findComponentPanelByUUID(const boost::shared_ptr<cryomesh::components::Connection> & component,
			std::map<boost::uuids::uuid, boost::shared_ptr< ConnectionActivityPanel > > & panelsMap) ;
